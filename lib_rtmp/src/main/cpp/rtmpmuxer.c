...
JNIEXPORT jint JNICALL
Java_com_simley_lib_1rtmp_RTMPMuxer_writeVideo(JNIEnv *env, jobject thiz, jbyteArray data_,
...
JNIEXPORT jint JNICALL
Java_com_simley_lib_1rtmp_RTMPMuxer_writeAudio(JNIEnv *env, jobject thiz, jbyteArray data_,
                                               jint offset, jint length, jlong timestamp) {
    jbyte *data = (*env)->GetByteArrayElements(env, data_, NULL);
...
JNIEXPORT jint JNICALL
Java_com_simley_lib_1rtmp_RTMPMuxer_read(JNIEnv *env, jobject thiz, jbyteArray data_, jint offset,
                                         jint size) {
...
Java_com_simley_lib_1rtmp_RTMPMuxer_isConnected(JNIEnv *env, jobject thiz) {
    return rtmp_is_connected() ? true : false;
}

JNIEXPORT jint JNICALL
Java_com_simley_lib_1rtmp_RTMPMuxer_close(JNIEnv *env, jobject thiz) {
    rtmp_close();
    return 0;
}